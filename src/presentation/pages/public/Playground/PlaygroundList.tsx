...
import { LANGUAGE_CONFIGS, LANGUAGE_ORDER } from "./templates/index";
...
export default function PlaygroundList() {
  ...
  async function createProject(language: Language) {
    ...
    try {
      ...
      const data = await playgroundUseCases.create({
  ...
  async function deleteProject(id: string) {
    if (deleting === id) {
      try {
        await playgroundUseCases.remove(id);
    ...
    <div className="min-h-screen text-[--color-text]">
      <div className="max-w-5xl mx-auto px-4 py-10">
        ...
        <section>
          ...
          {error && (
            <div className="mb-4 px-3 py-2 rounded-lg border border-red-500/30 bg-red-500/10 text-red-500 text-xs">
              {error}
            </div>
          )}
