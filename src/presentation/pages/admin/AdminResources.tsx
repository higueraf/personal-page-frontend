...
import { resourceUseCases } from "../../../infrastructure/factories/resource-module.factory";
...
export default function AdminResources() {
  ...
  const listQ = useQuery({ queryKey: ["admin-resources"], queryFn: () => resourceUseCases.list() });
  const resources: Resource[] = listQ.data?.data ?? [];

  const save = useMutation({
    mutationFn: (p: Partial<Resource>) => p.id ? resourceUseCases.update(p.id, p) : resourceUseCases.create(p),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["admin-resources"] }); setModal(null); },
  });
  const del = useMutation({
    mutationFn: (id: string) => resourceUseCases.delete(id),
