...
import type { VideoCourse, VCStatus } from "../../../domain/entities/video-course.entity";
...
const EMPTY: Partial<VideoCourse> = {
...
export default function AdminVideoCourses() {
  ...
  const [form, setForm]     = useState<Partial<VideoCourse>>(EMPTY);
  const [deleteId, setDeleteId] = useState<string | null>(null);

  const { data, isLoading } = useQuery({
    queryKey: ["admin-vcourses", q],
    queryFn: () => videoCourseUseCases.listCourses(q ? { search: q } : undefined),
  });
  const courses: VideoCourse[] = data?.data ?? [];
