...
type ItemType = ProfileItemType;
type ProfileItem = ProfileItemModel;
...
export default function AdminProfile() {
  ...
  const listQ = useQuery({ queryKey: ["admin-profile", filterType], queryFn: () => profileItemUseCases.list(filterType ? { type: filterType } : {}) });
  const items = listQ.data ?? [];

  const save = useMutation({
    mutationFn: (p: Partial<ProfileItem>) => p.id ? profileItemUseCases.update(p.id, p) : profileItemUseCases.create(p),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["admin-profile"] }); setModal(null); },
  });
  const del = useMutation({
    mutationFn: (id: string) => profileItemUseCases.delete(id),
