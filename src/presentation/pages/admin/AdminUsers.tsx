...
import { AssignExamPayload } from "../../../domain/entities/exam.entity";
...
async function fetchUsers(params: { status?: string; search?: string; page: number }) {
  const p: { page: number; page_size: number; status?: string; search?: string } = { page: params.page, page_size: 30 };
  if (params.status) p.status = params.status;
  if (params.search) p.search = params.search;
  const r = await adminUserUseCases.list(p);
  return r as unknown as { data: AdminUser[]; meta: { total_records: number; page: number; page_size: number } };
}

async function fetchRoles()        { return adminUserUseCases.listRoles() as unknown as Promise<Role[]>; }
async function fetchInstitutions() { return institutionUseCases.listAdmin() as unknown as Promise<Institution[]>; }
async function fetchStudyCourses(institution_id?: string) {
  const courses = await studyCourseUseCases.listAdmin(institution_id);
  return courses as unknown as StudyCourse[];
}
...
async function patchUser(id: string, body: {
  ...
  return adminUserUseCases.update(id, body) as unknown as Promise<AdminUser>;
}

async function assignExam(payload: AssignExamPayload) {
  return examUseCases.assign(payload);
}
