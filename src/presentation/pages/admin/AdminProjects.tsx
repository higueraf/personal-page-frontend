...
export default function AdminProjects() {
  ...
  const listQ = useQuery({ queryKey: ["admin-projects", page], queryFn: () => projectUseCases.list({ page, page_size: 20 }) });
  const projects: Project[] = listQ.data?.data ?? [];
  const meta = listQ.data?.meta;
  const totalPages = meta ? Math.ceil(meta.total_records / meta.page_size) : 1;

  const save = useMutation({
    mutationFn: (p: Partial<Project>) => p.id ? projectUseCases.update(p.id, p) : projectUseCases.create(p),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["admin-projects"] }); setModal(null); },
  });
  const del = useMutation({
    mutationFn: (id: string) => projectUseCases.delete(id),
