...
import type { Tutorial, TutorialStatus } from "../../../domain/entities/tutorial.entity";
import type { Institution } from "../../../domain/entities/institution.entity";
import type { StudyCourse } from "../../../domain/entities/study-course.entity";
...
const STATUS_CLS: Record<TutorialStatus, string> = {
  DRAFT: "badge--yellow", IN_REVIEW: "badge--blue",
  PUBLISHED: "badge--green", HIDDEN: "badge--yellow", ARCHIVED: "badge--red",
};

const LEVELS = ["Principiante", "Intermedio", "Avanzado"];
const EMPTY = { title: "", description: "", level: "Principiante", status: "DRAFT" as TutorialStatus, is_public: false, study_courses: [] as string[] };
...
export default function AdminTutorials() {
  ...
  const coursesQ = useQuery({
    ...
    queryFn: () => tutorialUseCases.list({ search: search || undefined }),
  });
  const instsQ = useQuery({ queryKey: ["admin-institutions"], queryFn: () => institutionUseCases.listAdmin() });
  const allCoursesQ = useQuery({ queryKey: ["admin-courses"], queryFn: () => studyCourseUseCases.listAdmin() });
  ...
  const createM = useMutation({
    mutationFn: (b: typeof EMPTY) => tutorialUseCases.create(b as any),
    onSuccess: () => { inv(); closeModal(); },
  });
  const updateM = useMutation({
    mutationFn: ({ pk, body }: { pk: string; body: typeof EMPTY }) => tutorialUseCases.update(pk, body as any),
    onSuccess: () => { inv(); closeModal(); },
  });
  const deleteM = useMutation({
    mutationFn: (pk: string) => tutorialUseCases.delete(pk),
    onSuccess: () => { inv(); setDeleteId(null); },
  });

  function openCreate() { setForm(EMPTY); setEditing(null); setModal("create"); }
  function openEdit(c: Tutorial) {
            ...
            <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <div style={{ display: "flex", gap: 12 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <select value={form.status} onChange={e => setForm(f => ({ ...f, status: e.target.value as TutorialStatus }))} style={inp}>
                    {(Object.entries(STATUS_LABEL) as [TutorialStatus, string][]).map(([v, l]) => <option key={v} value={v}>{l}</option>)}
                  </select>
                </div>
              </div>
              {/* Acceso público */}
