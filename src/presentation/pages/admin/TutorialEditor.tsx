...
import {
  tutorialUseCases,
  tutorialSectionUseCases,
  lessonUseCases,
  pageUseCases,
  contentBlockUseCases,
} from "../../../infrastructure/factories/tutorial-module.factory";
import type { TutorialSection } from "../../../domain/entities/tutorial-section.entity";
...
export default function TutorialEditor() {
  ...
  const courseQ = useQuery({
    ...
    queryFn: () => tutorialUseCases.get(courseId!),
    enabled: !!courseId,
  });

  const secQ = useQuery({
    queryKey: ["tutorial-section", courseId],
    queryFn: () => tutorialSectionUseCases.list(courseId!),
    enabled: !!courseId,
  });
  const section: TutorialSection | undefined = secQ.data?.[0];

  const lessonsQ = useQuery({
    queryKey: ["tutorial-lessons", section?.id],
    queryFn: () => lessonUseCases.list(section!.id),
    enabled: !!section?.id,
  });
  const lessons: Lesson[] = lessonsQ.data ?? [];

  // Sección automática
  const createSectionM = useMutation({
    mutationFn: () => tutorialSectionUseCases.create({ tutorial: courseId, title: "Contenido", order: 1 }),
  ...
  const handleDragStart = (e: React.DragEvent, lesson: Lesson, index: number) => {
  ...
  const reorderM = useMutation({
    mutationFn: (items: { id: string; order: number }[]) => lessonUseCases.reorder(items),
    onSuccess: () => {
  ...
  const pageQ = useQuery({
    ...
    queryFn: async (): Promise<TutorialPage | null> => {
      ...
      const pages = await pageUseCases.list(activeLessonId);
  ...
  const saveContent = useCallback(async (markdown: string) => {
    ...
    try {
      ...
      if (!page) {
        page = await pageUseCases.create({ lesson: activeLessonId, order: 1, estimated_minutes: 5, status: "PUBLISHED" });
      }
      const block = pageQ.data?.block ?? null;
      if (!block) {
        await contentBlockUseCases.create({ page: page.id, type: "markdown", order: 1, data: { markdown } });
      } else {
        // Siempre forzar type:markdown para que el serializer no use el tipo anterior
        await contentBlockUseCases.update(block.id, { type: "markdown", data: { markdown } });
      }
      setSaved(true);
  ...
  const deletePageM = useMutation({
    mutationFn: async (lessonId: string) => {
      ...
      for (const pg of pages) {
        const blocks = await contentBlockUseCases.list(pg.id);
        for (const bl of blocks) await contentBlockUseCases.delete(bl.id);
        await pageUseCases.delete(pg.id);
      }
      await lessonUseCases.delete(lessonId);
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["tutorial-lessons", section?.id] });
  ...
  const renameM = useMutation({
    mutationFn: ({ id, title }: { id: string; title: string }) => lessonUseCases.update(id, { title }),
