...
import type {
  VideoSection, VideoLesson, VideoType, VLStatus,
...
const EMPTY_LESSON: Partial<VideoLesson> = {
...
export default function CourseEditor() {
  ...
  const [form, setForm] = useState<Partial<VideoLesson>>(EMPTY_LESSON);
  ...
  const courseQ = useQuery({
    ...
    queryFn: () => videoCourseUseCases.getCourse(courseId!),
    enabled: !!courseId,
  });

  const sectionsQ = useQuery({
    queryKey: ["vc-sections", courseId],
    queryFn: () => videoCourseUseCases.listSections({ course_id: courseId }).then(r => r.data),
    enabled: !!courseId,
  });
  ...
  const lessonsQ = useQuery({
    ...
    queryFn: async () => {
      ...
      for (const s of secs) {
        const res = await videoCourseUseCases.listLessons({ section_id: s.id });
        all.push(...res.data);
      }
      return all;
    },
  ...
  const scheduleAutoSave = (data: Partial<VideoLesson>) => {
    setSaved(false);
    if (saveTimer.current) clearTimeout(saveTimer.current);
    saveTimer.current = setTimeout(() => doSave(data), 1500);
  };

  const doSave = useCallback(async (data: Partial<VideoLesson>) => {
    if (!activeLessonId) return;
    setSaving(true);
    try {
      await videoCourseUseCases.updateLesson(activeLessonId, data);
      setSaved(true);
      qc.invalidateQueries({ queryKey: ["vc-lessons", courseId] });
    } finally { setSaving(false); }
  }, [activeLessonId, courseId]);

  const handleField = (patch: Partial<VideoLesson>) => {
  ...
  const createSectionM = useMutation({
    mutationFn: (title: string) => videoCourseUseCases.createSection({
  ...
  const createLessonM = useMutation({
    mutationFn: (sectionId: string) => {
      ...
      return videoCourseUseCases.createLesson({
        section: sectionId,
        title: "Nueva lección",
  ...
  const deleteLessonM = useMutation({
    mutationFn: (id: string) => videoCourseUseCases.deleteLesson(id),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["vc-lessons", courseId] });
  ...
  const deleteSectionM = useMutation({
    mutationFn: async (sectionId: string) => {
      ...
      for (const l of sLessons) await videoCourseUseCases.deleteLesson(l.id);
      await videoCourseUseCases.deleteSection(sectionId);
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["vc-sections", courseId] });
      qc.invalidateQueries({ queryKey: ["vc-lessons", courseId] });
