...
type MsgStatus = ContactMessageStatus;
...
export default function AdminContact() {
  ...
  // ── Mensajes ──────────────────────────────────────────────────────────────
  const msgsQ = useQuery({ queryKey: ["admin-msgs", msgFilter], queryFn: () => contactUseCases.listMessages(msgFilter ? { status: msgFilter } : {}) });
  const msgs: ContactMsg[] = msgsQ.data?.data ?? [];
  const pending = msgs.filter(m => m.status === "PENDING").length;

  const patchMsg = useMutation({
    mutationFn: ({ id, status }: { id: string; status: MsgStatus }) => contactUseCases.updateMessageStatus(id, status),
  ...
  // ── Info de contacto ──────────────────────────────────────────────────────
  const infoQ = useQuery({ queryKey: ["admin-contact-info"], queryFn: () => contactUseCases.listInfo() });
  const contacts = infoQ.data ?? [];

  const saveInfo = useMutation({
    mutationFn: (f: Partial<ContactInfo>) => f.id ? contactUseCases.updateInfo(f.id, f) : contactUseCases.upsertInfo(f),
