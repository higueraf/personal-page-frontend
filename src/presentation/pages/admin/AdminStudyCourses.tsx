...
import { studyCourseUseCases, institutionUseCases } from "../../../infrastructure/factories/tutorial-module.factory";
...
export default function AdminStudyCourses() {
  ...
  const coursesQ = useQuery({ queryKey: ["admin-study-courses", filterInstId], queryFn: () => studyCourseUseCases.listAdmin(filterInstId || undefined) });
  const instQ = useQuery({ queryKey: ["admin-institutions"], queryFn: () => institutionUseCases.listAdmin() });

  const createMutation = useMutation({
    mutationFn: (body: { name: string; description?: string; institution_id?: string }) =>
      studyCourseUseCases.create(body),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: (data: { id: string; body: { name?: string; description?: string; institution_id?: string | null } }) =>
      studyCourseUseCases.update(data.id, data.body),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["admin-study-courses"] });
      closeModal();
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => studyCourseUseCases.delete(id),
